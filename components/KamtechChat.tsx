...
export default function KamtechChat() {
  ...
  const sendMessage = async (msg: string) => {
    ...
    try {
      ...
      setMessage("");
      // Optionnel: Fermer le chat ou afficher un message de succès
    } catch (error) {
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[9999] font-sans">
      {/* --- FENÊTRE DE CHAT --- */}
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[calc(100vw-2rem)] sm:w-[380px] h-[500px] bg-white dark:bg-[#0A0A0F] rounded-none shadow-2xl flex flex-col overflow-hidden border border-zinc-200 dark:border-zinc-800 transition-all duration-300">

          {/* Header avec dégradé Kamtech */}
          <div className="bg-gradient-to-b from-[#3D3FCC] to-[#2c29e0] p-6 relative">
            <button
            ...
            <div className="w-12 h-12 bg-white rounded-none flex items-center justify-center mb-4 shadow-sm">
          ...
          <div className="flex-1 p-5 bg-zinc-50 dark:bg-zinc-900/50 flex flex-col gap-4 overflow-y-auto">
            ...
            {/* Message automatique */}
            <div className="bg-white dark:bg-[#0A0A0F] p-4 rounded-none shadow-sm border border-zinc-100 dark:border-zinc-800 text-sm text-zinc-700 dark:text-zinc-300">
            ...
            <div className="flex flex-col gap-3 mt-2">
              <button
                ...
                className="group flex items-center justify-between bg-white dark:bg-[#0A0A0F] border border-zinc-200 dark:border-zinc-800 p-3 rounded-none hover:bg-[#3D3FCC] dark:hover:bg-[#6366F1] hover:text-white transition-all text-zinc-800 dark:text-zinc-200 text-sm font-medium"
              >
          ...
          <div className="p-4 bg-white dark:bg-[#0A0A0F] border-t border-zinc-100 dark:border-zinc-800 flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-zinc-100 dark:bg-zinc-900 rounded-none px-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#3D3FCC] dark:text-white"
            />
            <button
              onClick={handleSend}
              className="w-10 h-10 bg-[#3D3FCC] dark:bg-[#6366F1] rounded-none flex items-center justify-center text-white hover:opacity-90 transition-colors shadow-md"
            >
      ...
      <button
        ...
        className={`relative w-16 h-16 bg-[#3D3FCC] dark:bg-[#6366F1] rounded-none shadow-lg flex items-center justify-center transition-transform hover:scale-105 active:scale-95 ${
          isBouncing ? "animate-bounce" : ""
        }`}
      >
        {/* Pastille de notification rouge */}
        {showUnread && (
          <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-600 rounded-none border-2 border-white dark:border-zinc-950 z-10"></span>
        )}
