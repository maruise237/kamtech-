...
export function FaqSection() {
  ...
  return (
    <section id="faq" ref={sectionRef} className="bg-bg-2 py-24 md:py-32 overflow-hidden">
      <div className="max-w-3xl mx-auto px-6 md:px-12">
        ...
        <ScrollReveal progress={progress} direction="scale" offset={0} delay={0.2}>
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className={`rounded-none border transition-all duration-300 ${openIndex === i ? 'bg-bg-1 border-brand/30 shadow-md' : 'bg-bg-1 border-border hover:border-text-muted/30'}`}
              >
                <button
                  onClick={() => setOpenIndex(openIndex === i ? null : i)}
                  className="w-full flex items-center justify-between p-6 text-left"
                >
                  <span className="text-base font-semibold text-text-primary pr-4">{faq.q}</span>
                  <ChevronDown
                    size={20}
                    className={`flex-shrink-0 text-text-muted transition-transform duration-300 ${openIndex === i ? 'rotate-180 text-brand' : ''}`}
                  />
                </button>
                <div
